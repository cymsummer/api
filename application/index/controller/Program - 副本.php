...
class Program extends Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($cate)) {
            $cate_where = "and category_id=$cate";
        } else {
            $cate_where = "";
        }
        $category = Db::table("small_program_category")->where("category_type", $where)->field('id,category_name')->select();
        //获取文章id
        $cate_arr = Db::table("small_pro_cate")->distinct(true)->field("program_id,publish_time")->where("program_type=$where $cate_where")->page($page, 24)->order("publish_time desc")->select();
        if ($act == "hot") {
            foreach ($cate_arr as $v) {
                $arr[] = Db::table("small_program")->where("id=" . $v["program_id"])->field("id,program_title,program_subtitle,program_icon,program_see_num,program_audit_status")->find();
            }
            $data = array_map(create_function('$n', 'return $n["program_see_num"];'), $arr);
            array_multisort($data, SORT_DESC, SORT_NUMERIC, $arr);
        } elseif ($act == "new") {
            foreach ($cate_arr as $v) {
                $arr[] = Db::table("small_program")->where("id=" . $v["program_id"])->field("id,program_title,program_subtitle,program_icon,program_audit_status")->find();
            }
        }
        ...
        if (!empty($arr)) {
            foreach ($arr as $k => $v) {
                ...
                if ($v["program_audit_status"] == "0" || $v["program_audit_status"] == "1") {
                    unset($arr[$k]);
                }
            }
        } else {
...
//        $arr["label_name"]=$label_arr["label_name"];//获取标签信息
